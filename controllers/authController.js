...
const loginUser = async (req, res) => {   // router 로 부터 연결된 loginUser 비동기 함수 <== async key word (await 사용 가능)
  ...
  try {
    ...
    if (!user) {      // user 가 없을 경우
      return res.render('login', { error: '사용자 없음', title: 'Login' }); // error code 를 login.ejs 로 전달 :  error 객체 값 {error: '사용자 없음'}  
    }

    const match = await bcrypt.compare(password, user.password); // 전달 받은 password 와 user.password hash값을 비교 

    if (!match) {   // 일치하지 않을 경우
      return res.render('login', { error: '비밀번호 불일치', title: 'Login' }); // error code 를 login.ejs 로 전달 :  error 객체 값 {error: '비밀번호 불일치'}  
    }
    ...
    res.render('login', { error: '로그인 실패', title: 'Login' }); // error code 를 login.ejs 로 전달 :  error 객체 값 {error: '로그인 실패'}  
  }
};
